...
export default function EditMarketingRequest() {
  ...
  useEffect(() => {
    ...
    const fetchRequest = async () => {
      try {
        const res = await axios.get(`${API_URL}/marketing-requests/${id}`, {
        ...
        setFormData({
          ...
          request_type: data.request_type || 'featured',
        ...
        router.push('/marketing-requests');
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/marketing-requests/${id}`, payload, {
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }
      });
      router.push('/marketing-requests');
    } catch (err) {
  ...
  if (fetching) return <div style={styles.container}><p>Loading...</p></div>;

  return (
    <div style={styles.container}>
      <h1 style={styles.title}>Edit Marketing Request #{id}</h1>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <div style={styles.actions}>
          <button type="button" onClick={() => router.push('/admin/marketing-requests')} style={styles.cancelBtn}>
            Cancel
          </button>
...
const styles = {
  ...
  input: { width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #d1d5db', fontSize: '14px', outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.2s' },
  ...
  cancelBtn: { padding: '10px 20px', borderRadius: '8px', border: '1px solid #d1d5db', background: '#fff', color: '#374151', fontWeight: '600', cursor: 'pointer', fontSize: '14px' },
  submitBtn: { padding: '10px 24px', borderRadius: '8px', border: 'none', background: '#3b82f6', color: '#fff', fontWeight: '600', cursor: 'pointer', fontSize: '14px' },
};