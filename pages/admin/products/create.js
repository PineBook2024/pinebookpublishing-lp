...
export default function CreateProduct() {
  ...
  // Multiple product images (product_images table)
  const [productImages, setProductImages] = useState([]);
  const [productImagePreviews, setProductImagePreviews] = useState([]);
  
  // Single files (product_files table or direct columns)
  ...
  const handleSubmit = async (e) => {
    ...
    // Multiple product images - KEY FIX: use 'images[]' not 'images[index]'
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '20px 32px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <h1 style={{ margin: '8px 0 0', fontSize: 22, fontWeight: 700, color: '#1e293b' }}>New Product</h1>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                ...
                <input name="title" value={form.title} onChange={handleChange} style={inputStyle('title')} placeholder="Product title" />
                {errors.title && <span style={errorStyle}>{errors.title[0]}</span>}
              </div>
              <div>
                <label style={labelStyle}>Slug</label>
                <input name="slug" value={form.slug} onChange={handleChange} style={inputStyle('slug')} placeholder="auto-generated-if-empty" />
              </div>
              <div style={{ gridColumn: '1 / -1' }}>
                <label style={labelStyle}>Subtitle</label>
                <input name="subtitle" value={form.subtitle} onChange={handleChange} style={inputStyle('subtitle')} placeholder="Short subtitle" />
              </div>
            </div>
          </div>
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <input name="author_name" value={form.author_name} onChange={handleChange} style={inputStyle('author_name')} placeholder="Custom display name" />
              </div>
            </div>
          </div>
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
              <div>
                ...
                <input name="isbn" value={form.isbn} onChange={handleChange} style={inputStyle('isbn')} placeholder="978-3-16-148410-0" />
              </div>
              <div>
                <label style={labelStyle}>Language</label>
                <input name="language" value={form.language} onChange={handleChange} style={inputStyle('language')} placeholder="English" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" name="pages_count" value={form.pages_count} onChange={handleChange} style={inputStyle('pages_count')} placeholder="0" />
              </div>
              <div>
              ...
              <div>
                ...
                <input name="sku" value={form.sku} onChange={handleChange} style={inputStyle('sku')} placeholder="PROD-001" />
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
              <div>
                ...
                <input type="number" step="0.01" min="0" name="price" value={form.price} onChange={handleChange} style={inputStyle('price')} placeholder="0.00" />
                {errors.price && <span style={errorStyle}>{errors.price[0]}</span>}
              </div>
              <div>
                <label style={labelStyle}>Discount Price ($)</label>
                <input type="number" step="0.01" min="0" name="discount_price" value={form.discount_price} onChange={handleChange} style={inputStyle('discount_price')} placeholder="0.00" />
              </div>
              <div>
                <label style={labelStyle}>Stock Quantity</label>
                <input type="number" min="0" name="stock_quantity" value={form.stock_quantity} onChange={handleChange} style={inputStyle('stock_quantity')} placeholder="0" />
              </div>
            </div>
          </div>
          ...
          <div style={sectionStyle}>
            ...
            <p style={{ margin: '0 0 12px', fontSize: 12, color: '#64748b' }}>
              These are displayed in the product gallery. Stored in <code>product_images</code> table.
            </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 12 }}>
                {productImagePreviews.map((preview, idx) => (
                  <div key={idx} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                    ...
                    <button type="button" onClick={() => removeProductImage(idx)} style={{
          ...
          {/* ===== SINGLE FILES (Stored in product_files table or direct columns) ===== */}
          <div style={sectionStyle}>
            <h3 style={sectionTitle}>Product Files</h3>
            <p style={{ margin: '0 0 12px', fontSize: 12, color: '#64748b' }}>
              Single files per type. Can be stored in <code>product_files</code> table or direct product columns.
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <input 
                  type="file" 
                  accept="image/*" 
                  onChange={handleFileChange(setCoverImage, setCoverPreview)} 
                  style={{ display: 'none' }} 
                  id="cover-image" 
                />
                <label htmlFor="cover-image" style={{
                  display: 'block', padding: '20px', border: '2px dashed #e2e8f0',
                  borderRadius: 8, textAlign: 'center', cursor: 'pointer', color: '#64748b'
                }}>
                  {coverPreview ? (
                    <img src={coverPreview} alt="Cover" style={{ maxWidth: '100%', maxHeight: 120, borderRadius: 4 }} />
                  ) : (
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={handleFileChange(setEbookFile, null)} 
                  style={{ display: 'none' }} 
                  id="ebook-file" 
                />
                <label htmlFor="ebook-file" style={{
                  display: 'block', padding: '20px', border: '2px dashed #e2e8f0',
                  borderRadius: 8, textAlign: 'center', cursor: 'pointer', color: '#64748b'
                }}>
                  {ebookFile ? (
                    ...
                    <span>+ Click to upload ebook</span>
                  )}
                </label>
                {ebookFile && (
              ...
              <div>
                ...
                <input 
                  type="file" 
                  accept="image/*,.pdf" 
                  onChange={handleFileChange(setPreviewFile, null)} 
                  style={{ display: 'none' }} 
                  id="preview-file" 
                />
                <label htmlFor="preview-file" style={{
                  display: 'block', padding: '20px', border: '2px dashed #e2e8f0',
                  borderRadius: 8, textAlign: 'center', cursor: 'pointer', color: '#64748b'
                }}>
                  {previewFile ? (
                    ...
                    <span>+ Click to upload preview</span>
                  )}
                </label>
                {previewFile && (
              ...
              <div>
                ...
                <input 
                  type="file" 
                  accept=".pdf" 
                  onChange={handleFileChange(setSamplePdf, null)} 
                  style={{ display: 'none' }} 
                  id="sample-pdf" 
                />
                <label htmlFor="sample-pdf" style={{
                  display: 'block', padding: '20px', border: '2px dashed #e2e8f0',
                  borderRadius: 8, textAlign: 'center', cursor: 'pointer', color: '#64748b'
                }}>
                  {samplePdf ? (
                    ...
                    <span>+ Click to upload sample PDF</span>
                  )}
                </label>
                {samplePdf && (
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
              <div>
                ...
                <input type="number" step="0.01" min="0" max="5" name="average_rating" value={form.average_rating} onChange={handleChange} style={inputStyle('average_rating')} placeholder="0.00" />
              </div>
              <div>
                <label style={labelStyle}>Total Reviews</label>
                <input type="number" min="0" name="total_reviews" value={form.total_reviews} onChange={handleChange} style={inputStyle('total_reviews')} placeholder="0" />
              </div>
              <div>
                <label style={labelStyle}>Total Sales</label>
                <input type="number" min="0" name="total_sales_count" value={form.total_sales_count} onChange={handleChange} style={inputStyle('total_sales_count')} placeholder="0" />
              </div>
            </div>
          </div>
          ...
          <div style={sectionStyle}>
            ...
            <textarea name="admin_notes" value={form.admin_notes} onChange={handleChange} rows={4} style={{ ...inputStyle('admin_notes'), resize: 'vertical' }} placeholder="Internal notes..." />
          </div>
